...
namespace ChurchManager.DataImporter
{
    class Program
    {
        ...
        private static readonly ChurchManagerDbContext DbContext = new(_options);
        ...
        public static IEnumerable<IEnumerable<object>> ImportExcel(Stream stream, bool skipFistRow)
        {
            ...
            if (!DbContext.Group.Any())
            {
                var parentGroups = groups
                    .Where(import => import.ParentGroup.IsNullOrEmpty())
                    .Select(x => new Group
                {
                    Name = x.Name,
                ...
                var childGroups = groups
                    ...
                    .Select(import => new Group
                    {
                        ...
                        GroupTypeId = 1,
                        ParentGroupId = DbContext.Group.FirstOrDefault(x => x.Name ==  import.ParentGroup)?.Id
                    });

                DbContext.AddRange(parentGroups);
                var inserted = DbContext.SaveChanges();
                Console.WriteLine($"Groups added: {inserted}");


                DbContext.AddRange(childGroups);
